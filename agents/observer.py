...
from agents.models import ObserverRequest, ObserverResult, RawResponse, SteerReminder, Verdict
...
class CMDIJudge(Judge):
    ...
    def judge(
        ...
        if match := _CMDI_BASE64.search(diff_text):
...
class SQLiJudge(Judge):
    ...
    def judge(
        ...
        # DB 错误快速匹配
        if diff_text and (match := _DB_ERROR_PATTERNS.search(diff_text)):
            error_snippet = match.group()[:300]
            return Verdict(
                payload=payload,
                is_bypass=True,
...
class Log4jJudge(Judge):
    ...
    def judge(
        ...
        if search_text and (match := _JNDI_EXCEPTION_PATTERNS.search(search_text)):
...
# ============================================================
# StuckDetector - 启发式卡循环检测器（零 LLM 调用）
# ============================================================


class StuckDetector:
    """旁路纠偏检测器。维护滑动窗口分析近期轮次轨迹。

    不接收 payload 语义，只看 dimension、bypass/blocked 计数、status_code。
    """

    def __init__(self, window_size: int = 6):
        self._window: collections.deque[dict] = collections.deque(maxlen=window_size)

    def record_round_trace(
        self,
        round_num: int,
        dimension: str,
        bypass_count: int,
        blocked_count: int,
        dominant_status_code: int | None,
    ) -> None:
        self._window.append({
            "round_num": round_num,
            "dimension": dimension,
    ...
    def detect_stuck(self, available_dimensions: list[str] | None = None) -> SteerReminder | None:
        if len(self._window) < 3:
            return None

        window = list(self._window)

        # Rule 1: 维度停滞 — 连续 4+ 轮同一维度且 0 绕过
        if len(window) >= 4:
            last_dims = [w["dimension"] for w in window[-4:]]
            last_bypasses = [w["bypass_count"] for w in window[-4:]]
            if len(set(last_dims)) == 1 and all(b == 0 for b in last_bypasses):
                current_dim = last_dims[0]
                suggested = self._next_dimension(current_dim, available_dimensions)
                return SteerReminder(
                    message=f"连续 {len(last_dims)} 轮使用 {current_dim} 维度且 0 绕过 — 建议切换维度",
                    suggested_dimension=suggested,
                    urgency="high",
                    source_pattern="dimension_stagnation",
                )

        # Rule 2: 状态码单调 — 连续 4+ 轮同一主导状态码
        if len(window) >= 4:
            recent_codes = [w["dominant_status_code"] for w in window[-4:]]
            if (
                recent_codes[0] is not None
                and len(set(recent_codes)) == 1
            ):
                code = recent_codes[0]
                return SteerReminder(
                    message=f"WAF 连续 {len(recent_codes)} 轮返回 HTTP {code} — 当前策略被签名匹配",
                    urgency="medium",
                    source_pattern="status_code_monotony",
                )

        # Rule 3: 绕过后退化 — 早期有绕过但最近 3+ 轮全拦截
        if len(window) >= 4:
            early = window[: len(window) // 2]
            recent = window[-3:]
            had_bypass = any(w["bypass_count"] > 0 for w in early)
            recent_all_blocked = all(
                w["bypass_count"] == 0 and w["blocked_count"] > 0 for w in recent
            )
            if had_bypass and recent_all_blocked:
                return SteerReminder(
                    message="早期有绕过成功但最近 3 轮全拦截 — WAF 可能已自适应或签名更新",
                    urgency="medium",
                    source_pattern="bypass_regression",
                )

        return None

    @staticmethod
    def _next_dimension(current: str, available: list[str] | None) -> str | None:
        if not available:
            return None
...
class Observer:
    ...
    def evaluate(self, request: ObserverRequest) -> ObserverResult:
        ...
        if needs_oob and request.oob_received is not None:
            # 优先使用 provider 直接 poll 的结果
            oob_received = request.oob_received
        elif (
            needs_oob
        ...
        # 6. 旁路纠偏：记录轨迹并检测卡循环
        status_codes = [v.status_code for v in verdicts if v.status_code > 0]
        dominant_code = Counter(status_codes).most_common(1)[0][0] if status_codes else None
        self._stuck_detector.record_round_trace(
            round_num=request.round_num,
            dimension=request.dimension,
            bypass_count=bypass_count,
            blocked_count=blocked_count,
            dominant_status_code=dominant_code,
        )
        steer = self._stuck_detector.detect_stuck()
